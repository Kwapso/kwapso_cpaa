...
- **The screen engine:** `/t/<teamId>/<module>/<id>` is one client-resolved shell (`web/components/deep-link-screen.tsx`); recipes in `web/lib/screens.ts`; nav in `web/lib/pages.ts`. Learning + Tickets also have clean top-level URLs (`/learning`, `/tickets`). The Tickets section's URL segment and label say **tickets**; the permission module, the table and the API path are still `help` (see `MODULE_PERMISSION` in `web/lib/screens.ts`). Engine-expressible screens → a recipe; bespoke screens → a host-composed component (like `role-detail.tsx`).
