...
- **The UI library is lego, and the lego now lives here.** Primitives and collections come from `shared/ui/`, imported as `@shared/ui/registry/primitives/button/button`; `web/` and `web-portal/` assemble recipes from them. It used to be the npm package `@kwapso/ui`, installed from a separate repo; it was **vendored into this repo on 2026-08-22** because the app is being re-themed and a token remap cannot change a component's SHAPE (the design kit's secondary button is a filled button with no border in any state). Read `shared/ui/README.md` before you touch it. **The layering did not merge**, only the address changed: a primitive is still generic, app-agnostic lego, and a this-app-specific control still belongs in `web/components/`, not in `shared/ui/`. If a primitive needs changing, change it in `shared/ui/` directly, that fork already happened, deliberately, once. **What must never happen is editing UPSTREAM**: `swift-struck-ui` is a live dependency of other Swift Struck products, so never push there, never open a PR from here, never "sync back" a fix. This is a copy; their copy is untouched.
