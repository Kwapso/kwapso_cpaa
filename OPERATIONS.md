...
- Login codes: **a code appears NOWHERE but the user's inbox — in any environment.** The old `DEV_ECHO_CODES` echo (code in the response + a toast) is DELETED, code path and config var both, so configuration can't re-enable it. Automated runs (the smoke, the e2e suite) sign in through the **admin test-login door** instead: `POST /api/auth/admin/test-login` `{email}` with an `x-admin-key` header mints a NORMAL hashed-at-rest code (same TTL, same attempt cap, same per-hour throttle as the real send path) and returns it once; the normal verify door consumes it. Gated by an `ADMIN_KEY` secret **on the auth worker** that FAILS CLOSED when unset. **Set it on STAGING only** (`cd workers/auth && npx wrangler secret put ADMIN_KEY --env staging`) — the holder can sign in as ANY account on that environment, so **production must never have it** (production auth simply has no test door). The smoke + e2e read it from the environment: `export ADMIN_KEY=…` before `npm run smoke:staging` / the e2e suite.
