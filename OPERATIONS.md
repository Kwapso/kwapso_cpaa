...
| content (`workers/content`) | kwapso-content-staging | kwapso-content | BUILT 2026-06-23. The team-DB content modules: **Learning** (how-to items + per-user "mark done" progress) + **Tickets** (account-fenced tickets + threaded replies, fixed status lifecycle; the permission key, tables and path are still `help` — DATA-MODEL.md says why). Routes `/api/content/*`. Binds AUTH + REALTIME + the core DB (gating) + two R2 buckets (`LEARNING_MEDIA`, `HELP_MEDIA`). No cron |
