...
| 4.9 | Who can see everyone else's tasks | **DONE**. **CHANGED**: a configurable permission, Aurora's answer. It is an ordinary row on the permission sheet, **Everyone's tasks**, off for every role but Admin. Without it the tasks screen still opens and shows the ones assigned to you, narrowed at the DOOR, so all eight counts and the progress bar come back narrowed with it, and naming a colleague in the request gets you your own list rather than theirs |
...
| 7.2 | The "Under this account" tab goes | **DONE**, gone, with the panel, the fetch and the cache key behind it. Contacts is the survivor. The parent pointer is untouched: a company still sits under its holding company, the form still moves it, and the header still links one tap up the tree |
...
| 7.4 | Only a contact can hold a login | **DONE**. **CHANGED**: Aurora's answer, over your "both levels". The row-writing half already refused a company; what it did not do was refuse it FIRST, so a company with an info@ address got "hasn't signed in here yet. Ask them to sign in once", instructions for making a company into a login-holder, from a door that was always going to say no. The first thing the door reads now decides it |
...
| 7.15 | A knowledge tab inside an account | **DONE**, the same seam an app's carries (8.9), and the same two things travel: the account names the COMPARTMENT, so a question typed here can never be answered out of another client's material, and the record's own details are prepended to the question and **shown on screen** so you can see what it asked |
...
| 12.3 | Control over who can see what inside it | **DONE**, a source now has three answers to "who may read this", not two: the whole team, **the people staffed to one app**, or only you. The middle one rides the fence the app record already has (8.11, its staff, plus an admin), so nobody has a second scheme to learn and there is no access-control list to keep in step. Decided in the DATABASE, never in the vector index (R26): the index narrows, and every passage is read back under the caller's own fence, so putting somebody on an app grants them sight of it that instant with nothing re-indexed. Choosing an app you are not on is refused in words, rather than filing something you would be the first person unable to read |
...
| 13.15 | A rule that fails the build when a new string escapes the catalogue | **DONE**. **R28**, in RULES.md, the registry and a check, all three (the build refuses a law missing any of them). It re-runs the real extractor over both front doors rather than describing what it does, and it names two failures separately: a sentence MISSING from the catalogue ships in English to somebody who chose German, and an entry matching no string in the app is an ORPHAN translated on every build for nothing. It went red on its first run and caught two real strings |
...
| 14.12 | Google comes into step when you open the app, not only on a button | **DONE**, one request when the shell mounts, behind first paint, as the signed-in person through the same door the button opens. Nothing acts as anybody and nothing reaches the cron: everything Google is still read with one person's own token. The door carries a five-minute floor read off the sweep's own `last_run_at`, and the floor is asked for by the automatic caller rather than imposed on everyone, so re-shelving a folder and pressing sync still works exactly as it did. Somebody without the Google right makes no request at all; somebody who has connected nothing gets an empty answer and no error |
