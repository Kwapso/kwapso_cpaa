...
export const RADIUS_EXCEPTION: Record<string, string> = {
  "rounded-pill":
    "The design kit's own pill word (tokens.css `--radius-pill`). It IS `rounded-full` — the value is the pill — but spelled through a token so a theme can in principle soften it, and the kit writes it ~180 times. Admitting the kit's spelling beats rewriting a vendored dependency into ours, and the hand-edit guard (VERSION.json) forbids that rewrite anyway.",
  "rounded-select":
