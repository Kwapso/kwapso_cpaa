...
const TRIGGER_BASE = [
  // The kit's reset line on every bare control.
  "inline-flex cursor-pointer appearance-none items-center justify-center gap-2",
  "border-0 bg-transparent text-inherit [font:inherit]",
 ...
 * nothing to clip against and nothing for the tab to match. It takes `--card`
 * at `--radius`, and `z-2` — above the inactive tabs' `z-1`, below the active
 * tab's `z-3`, which are chapter 24.3's own three numbers.
 *
 * THREE BREAKPOINTS
  ...
  return (
    <TabsPrimitive.Content
      ...
      className={cn(
        "motion-tab-panel-in",
        resolved === "folder" &&
          "relative z-[2] rounded-[var(--radius)] bg-card p-6 text-card-foreground",
        className,
      )}
      {...props}
    />
