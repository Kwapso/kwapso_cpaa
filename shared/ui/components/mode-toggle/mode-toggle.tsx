...
const SEGMENT_BASE = [
  // The kit's reset line on a bare control. NOT `[font:inherit]`: Tailwind
  // emits that arbitrary property AFTER the named utilities, so the shorthand
  // was overriding the `text-sm leading-none` below (and the selected
  // segment's `font-medium`) with the surrounding type. Preflight already
  // gives a <button> `font: inherit`; the named classes then own the step.
