...
/** A card of hairline-separated rows, or standalone rows on the page. */
export type ListVariant = "panel" | "rows";
      ...
      >
        {rows.map((row, index) => {
            ...
            <>
              ...
              <span data-slot="list-body" className="flex min-w-0 flex-1 flex-col">
                <span
                  ...
                  className={cn("truncate", TITLE_TYPE[density])}
                >
                  {row.title}
                </span>
                {row.description !== undefined && row.description !== null ? (
                  /* `.kw-list__meta` — the caption step in tertiary ink, 4
                     ...
                     nearest ladder step — GAPS-COL3 LST-3). */
                  <span
                    data-slot="list-description"
                    className="mt-1 truncate text-caption text-ink-tertiary"
                  >
                    {row.description}
                  </span>
                ) : null}
          ...
          const rowClasses = cn(
            ...
            // The 8% hairline under every row but the last (ch17).
