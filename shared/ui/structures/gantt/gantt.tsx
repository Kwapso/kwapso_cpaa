/* ============================================================================
   ...
   · THE THREE TONES ARE `--chart-1` / `--chart-2` / `--chart-3` — sky, forest,
     poppy, in the kit's chart order. There is no fourth: `--chart-4` and
     `--chart-5` repeat 1 and 2 (spec, still-open register) and a timeline that
     used them would draw two tones that mean different things and look the
     same. Ink on a bar is `--ink-on-accent`, the accent law as a token.
   · NEVER MANGO. Mango is a fill, never a data colour and never a status.
...
export interface GanttBar {
   ...
   */
  span?: number;
  /** Sky, forest or poppy. Defaults to `build`; mango is never an option. */
  tone?: GanttTone;
  /**
  ...
  (
    ...
    const renderBar = (bar: GanttBar, lane: GanttLane, index: number, placed: boolean) => {
      ...
      const skin = bar.disabled
        ...
        : cn(BAR_TONE[bar.tone ?? "build"], "text-ink-on-accent", pressable && "motion-hover-lift");
      ...
      return pressable ? (
        <button
          ...
          className={cn("appearance-none [font:inherit]", classes)}
        >
          {inner}
        </button>
      ) : (
