...
// the Dropdown values screen and nothing in the code knows they exist. These five
// are different: a worker pick-or-creates into them (the way a learning article's
// category is pick-or-created into "Learning category"), or the legacy migration
// lands a set of rows in them. So the string has to be said in ONE place, or the
...
// three module vocabularies do not — those fill themselves from the first value
// anybody writes, or from the rows the legacy import brings.

export const SELECTABLE_GROUPS = {
  /** A marketing post's channel — the legacy `channels` table, six rows of bare
   * label, which is why it is a vocabulary here rather than a table of its own. */
  channel: "Marketing channel",
  /** Where a marketing post has got to: drafted, scheduled, published. */
