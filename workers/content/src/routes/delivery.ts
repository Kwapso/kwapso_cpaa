// Delivery-method routes — programmes and meeting purposes, two tables under
// one module and one permission. Same shape as its siblings; R21 on every door,
// both halves, because how the agency runs its own delivery is the agency's.
...
export async function getPrograms(request: Request, env: Env): Promise<Response> {
  const { cfg, guard } = await gated(request, env, "delivery", "read")
  await refusePortalCaller(cfg, guard)
  const programs = await listPrograms(cfg, guard)
  const id = queryText(new URL(request.url).searchParams.get("id"), "Id") // ?id= → one programme
  // R16: the exact server total rides every list response.
  return json({
    programs: id ? programs.filter((p) => p.id === id) : programs,
    total: await countPrograms(cfg, guard),
  })
}

export async function getProgramsExport(request: Request, env: Env): Promise<Response> {
  const { cfg, guard } = await gated(request, env, "delivery", "read")
  await refusePortalCaller(cfg, guard)
  const { rows, complete } = await listProgramsForExport(cfg, guard)
  if (!complete)
    return exportTooLarge(EXPORT_HARD_CAP, "programmes", "Archive the ones you no longer run, then export again.")
  const csv = toCsv(
    ["name", "description", "sequence", "active", "created_at", "created_by", "updated_at", "updated_by"],
    rows.map((p) => [
      p.name, p.description, p.sequence,
      p.deactivated_at == null, p.created_at, p.creator_name, p.updated_at, p.editor_name,
    ])
  )
  return csvResponse("programmes.csv", csv)
}

export async function postCreateProgram(request: Request, env: Env): Promise<Response> {
  const { actor, cfg, guard, body } = await gatedBody<ProgramInput>(request, env, "delivery", "create")
  await refusePortalCaller(cfg, guard)
  requireText(body.name, "Name", TEXT_LIMITS.short)
  const id = await createProgram(cfg, guard, actor, body)
  await publishChange(env, guard.teamId, "programs", id, "add")
  return json({ programs: await listPrograms(cfg, guard), total: await countPrograms(cfg, guard) })
}

export async function postUpdateProgram(request: Request, env: Env): Promise<Response> {
  const { actor, cfg, guard, body } = await gatedBody<ProgramInput & { id?: string }>(request, env, "delivery", "edit")
  await refusePortalCaller(cfg, guard)
  const id = requireText(body.id, "Programme", TEXT_LIMITS.short)
  requireText(body.name, "Name", TEXT_LIMITS.short)
  await updateProgram(cfg, guard, actor, id, body)
  await publishChange(env, guard.teamId, "programs", id)
  return json({ programs: await listPrograms(cfg, guard), total: await countPrograms(cfg, guard) })
}

/** Archive / restore a programme — never deleted. Gated delivery:delete. */
export async function postSetProgramActive(request: Request, env: Env): Promise<Response> {
  ...
  const changed = await setProgramActive(cfg, guard, actor, id, body.active)
  if (changed) await publishChange(env, guard.teamId, "programs", id)
  return json({ programs: await listPrograms(cfg, guard), total: await countPrograms(cfg, guard) })
}
