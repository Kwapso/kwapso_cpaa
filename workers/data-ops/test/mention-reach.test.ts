...
// THIS SUITE ONCE PROVED THE OPPOSITE, AND THE STORY IS THE POINT.
//
// A security sweep found that the assistant could email up to fifty teammates,
// in the caller's name, with no confirm panel. A later lane refuted it: the
// builder dropped `taggedUserIds`, so the mention never left the worker. Both
// were right about the tree they read — between them, R22 (body-field parity)
// made the tool EXPOSE the field, because the door reads it and a machine must
// not be handed a narrower contract than the screen has.
//
// So the refutation expired the moment parity landed, and the forward-looking
// rule below — written by the lane that refuted it — is what caught that. The
// rule outlived the conclusion. That is what a rule is for.
describe("a reply may carry an audience, and an audience must be asked about", () => {
  ...
  it("forwards the mentions its door accepts — R22, the whole contract", () => {
    const body = reply.buildBody!({ helpId: "H1", body: "hi", taggedUserIds: ["U1", "U2"] })
    expect(body.taggedUserIds, "a machine gets the door's contract, not a subset").toEqual(["U1", "U2"])
  })

  it("a plain reply does NOT stop to ask", () => {
    // The assistant answering a ticket is the ordinary case and must stay fluent.
    expect(requiresConfirm(reply, { helpId: "H1", body: "on it" })).toBe(false)
  })

  it("a reply that addresses people DOES stop to ask", () => {
    // Gated on help:read — the lowest bar in the catalogue — and every mention
    // sends branded mail from the verified sender carrying a preview of text the
    // model was handed, which can have been typed by whoever raised the ticket.
    expect(requiresConfirm(reply, { helpId: "H1", body: "hi", taggedUserIds: ["U1"] })).toBe(true)
    expect(requiresConfirm(reply, { helpId: "H1", body: "hi", taggedUserIds: [] }), "an empty list is nobody").toBe(
      false
    )
  })
})
