...
export function AppMeetingsPanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      <ToolbarRow actions={onNew && <AddButton label={t("Arrange a meeting")} onClick={onNew} />} />
      {rows.length === 0 ? (
        ...
        <CollectionEmptyState title={t("No meetings about this app yet.")} onCreate={onNew} />
      ) : (
        <RowList>
          {rows.map((m) => (
...
export function AppTicketsPanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      <ToolbarRow actions={onNew && <AddButton label={t("Raise a ticket")} onClick={onNew} />} />
      {rows.length === 0 ? (
        // No `help`/tickets import target exists.
        <CollectionEmptyState title={t("Nothing has been raised about this app yet.")} onCreate={onNew} />
      ) : (
        <RowList>
          {rows.map((ticket) => (
...
export function TodosPanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* THE TABS ALONE, THE TOOLBAR BELOW THEM (client ruling, 2026-08-31):
          this used to put "Ask for something" ON the tabs' own row, exactly
          the shape corrected everywhere else in the app. `ToolbarRow` pins it
          right, on its own row, still above the list. */}
      <div className="flex flex-col gap-2">
        {tabs}
        <ToolbarRow actions={onNew && <AddButton label={t("Ask for something")} onClick={onNew} />} />
      </div>

      {q.error ? (
        <p className="text-destructive text-sm">{t("Couldn't load the inputs.")}</p>
      ) : q.data === undefined ? (
        <Skeleton variant="list" lines={3} />
      ) : q.data.length === 0 ? (
        // No import target for to-dos — they are asked of a client one at a
        // time. The "done" pile has nothing to ADD (it is what came BACK),
        // so its empty state carries no button at all.
        <CollectionEmptyState
          ...
          onCreate={view === "open" ? onNew : undefined}
        />
      ) : view === "done" ? (
