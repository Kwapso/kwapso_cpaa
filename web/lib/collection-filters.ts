...
export const COLLECTION_FILTERS: Record<string, CollectionFacet[]> = {
  ...
  meetings: [
    ...
    // TWO WORDS, not three. The diary's rows also carry "Cancelled", which is
    // `active = 0` rather than a status — and this door parses no such
    // parameter, so offering it would be a control that cannot be honoured.
    // Named here rather than left quiet: it wants a filter on the meetings door,
    // which belongs to whoever is rewriting that module.
    {
      field: "status",
      label: "Status",
      options: [
